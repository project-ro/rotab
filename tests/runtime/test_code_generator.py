...
def template_and_context(request) -> tuple[TemplateNode, ValidationContext, List[str], List[str], str]:
    ...
    if backend == "pandas":
        ...
        expected_process = [
            ...
            "def step_mutate_step_full_process(user):",
            "    mutated = user",
            "    mutated = mutated.filter(parse('age > 20'))",
            '    mutated = mutated.with_columns(parse("""\n'
            "        log_age = log(age)\n"
            "        age_bucket = age // 10\n"
            '        """))',
            "    mutated = mutated.select(['user_id', 'log_age', 'age_bucket'])",
            "    return mutated",
            "",
            "",
            "def step_transform_step_full_process(mutated):",
            "    result = transform_func(mutated)",
            "    return result",
            "",
            "",
            "def full_process():",
            '    user = pl.scan_csv("user.csv", dtypes={"user_id": pl.Utf8, "age": pl.Int64})',
            "    mutated = step_mutate_step_full_process(user)",
            "    result = step_transform_step_full_process(mutated)",
            '    result = result.with_columns(pl.col("user_id").cast(pl.Utf8))',
            '    result = result.with_columns(pl.col("log_age").cast(pl.Float64))',
            '    result = result.with_columns(pl.col("age_bucket").cast(pl.Int64))',
            '    result.collect().write_csv("result.csv")',
            "    return result",
            "",
            "",
            'if __name__ == "__main__":',
