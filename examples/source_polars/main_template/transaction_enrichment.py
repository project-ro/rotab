...
def step_filter_users_main_transaction_enrichment(filtered_users):
    if True:
        ...
        filtered_users_main = filtered_users_main.with_columns(
            parse(
                """
            log_age = log(age)
...
def step_enrich_transactions_transaction_enrichment(enriched):
    ...
    final_output = final_output.with_columns(
        parse(
            """
...
def transaction_enrichment():
    ...
    filtered_users = pl.scan_csv(
        "data/outputs/filtered_users.csv", dtypes={"user_id": pl.Utf8, "age": pl.Int64, "age_group": pl.Int64}
    )
    filtered_transactions = pl.scan_csv(
        "data/outputs/filtered_transactions.csv",
        dtypes={"user_id": pl.Utf8, "amount": pl.Int64, "is_large": pl.Boolean},
    )
