...
def test_context_builder():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # スキーマファイルをダミーで書き出し
        with open(os.path.join(schema_dir, "dummy_schema.yaml"), "w") as f:
        ...
        dummy_template = TemplateNode(
            ...
            processes=[
                ProcessNode(
                    name="dummy_process",
                    inputs=[InputNode(name="input_df", io_type="csv", path="dummy.csv", schema="dummy_schema")],
                    outputs=[OutputNode(name="output_df", io_type="csv", path="out.csv", schema="dummy_schema")],
                    steps=[],
                )
            ],
        )
        ...
        assert "dummy_schema" in context.schemas
